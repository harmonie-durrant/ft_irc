...
void Invite::execute(Client* client, std::vector<std::string> args)
{
    ...
    if (!target) 
    {
        client->send_response(401, _server, client, targetNick + " :No such nick/channel");
        return;
    }
    //Si le channel n'existe pas on peut quand meme envoyer une invitation (bizarerie RFC2812)
    if (!channel) 
    {
        client->send_response(-1, _server, client, "You have invited " + targetNick + " to " + channelName);
        target->send_response(-1, _server, target, ":" + client->getNickname() + " INVITE " + targetNick + " " + channelName);
        return;
    }
    //Si le client est deja dans le channel
    if (channel->isClient(target))
    {
        client->send_response(443, _server, client, targetNick + " " + channelName + " :is already on channel");
        return;
    }
